import { useReducer } from "react";
import localforage from "localforage";
import { toast } from "react-toastify";
import apiBaseUrl from "@/helpers/apiBase";

const initialState = {
  products: [],
  cart: [],
  cartTotal: 0,
  cartQuantity: 0,
  order: [],
};

const actions = Object.freeze({
  ADD_TO_CART: "ADD_TO_CART",
  GET_PRODUCTS: "GET_PRODUCTS",
  REMOVE_FROM_CART: "REMOVE_FROM_CART",
  CLEAR_CART: "CLEAR_CART",
  ADD_QUANTITY: "ADD_QUANTITY",
  REDUCE_QUANTITY: "REDUCE_QUANTITY",
  SET_QUANTITY: "SET_QUANTITY",
  PREFILL_CART: "PREFILL_CART",
  ADD_PRODUCT: "ADD_PRODUCT",

});

const reducer = (state, action) => {
  // ADD PRODUCT
  if (action.type === actions.ADD_PRODUCT) {
  const updatedProduct = {
    ...action.product,
    addedToCart: false, // ensure default flag
  };
  return {
    ...state,
    products: [...state.products, updatedProduct],
  };
}

  // GET PRODUCTS
  if (action.type == actions.GET_PRODUCTS) {
    if (action.backed_up_cart == []) {
      return { ...state, products: action.products };
    }
    // prefil cart
    const cartTotal = action.backed_up_cart.reduce(
      (acc, item) => acc + item.price * item.quantity,
      0
    );
    const cartQuantity = action.backed_up_cart.reduce(
      (acc, item) => acc + item.quantity,
      0
    );

    const updatedProducts = action.products.map((product) => {
      const cartItem = action.backed_up_cart.find(
        (item) => item.id === product.id
      );
      if (cartItem) {
        return { ...cartItem, addedToCart: true };
      } else {
        return product;
      }
    });
    return {
      ...state,
      products: updatedProducts,
      cart: action.backed_up_cart,
      cartQuantity,
      cartTotal,
    };
  }
  // ADD TO CART
  if (action.type == actions.ADD_TO_CART) {
    const product = state.products.find(
      (item) => item.id == action.product
    );
    if (!product) {
      return state;
    }

    const quantityToAdd = Math.max(1, Number(action.quantity) || 1);
    const existingCartItem = state.cart.find((item) => item.id == product.id);

    const updatedCart = existingCartItem
      ? state.cart.map((item) =>
          item.id === product.id
            ? { ...item, quantity: item.quantity + quantityToAdd, addedToCart: true }
            : item
        )
      : [...state.cart, { ...product, quantity: quantityToAdd, addedToCart: true }];

    const updatedProducts = state.products.map((item) =>
      item.id === product.id
        ? { ...item, addedToCart: true, quantity: (existingCartItem?.quantity || 0) + quantityToAdd }
        : item
    );

    localforage.setItem("cartItems", updatedCart);

    return {
      ...state,
      products: updatedProducts,
      cart: updatedCart,
      cartQuantity: state.cartQuantity + quantityToAdd,
      cartTotal: state.cartTotal + product.price * quantityToAdd,
    };
  }
  // Remove from cart
  if (action.type == actions.REMOVE_FROM_CART) {
    const product = state.products.find(
      (item) => item.id == action.product
    );
    const removedItem = state.cart.find((item) => item.id == action.product);
    const removedQuantity = removedItem?.quantity || 0;
    const newCart = state.cart.filter(
      (item) => item.id != action.product
    );
    const updatedProduct = product
      ? { ...product, addedToCart: false, quantity: 0 }
      : null;

    localforage.setItem("cartItems", newCart);

    let newCartTotal = 0;
    newCart.forEach((item) => {
      newCartTotal += item.price * item.quantity;
    });

    return {
      ...state,
      products: updatedProduct
        ? state.products.map((p) => (p.id === product.id ? updatedProduct : p))
        : state.products,
      cart: newCart,
      cartQuantity: Math.max(0, state.cartQuantity - removedQuantity),
      cartTotal: newCartTotal,
    };
  }

  // add quantity
  if (action.type == actions.ADD_QUANTITY) {
    const product = state.cart.find((item) => item.id == action.product);
    if (!product) {
      return state;
    }
    const updatedCart = state.cart.map((item) =>
      item.id === product.id
        ? { ...item, quantity: item.quantity + 1 }
        : item
    );

    localforage.setItem("cartItems", updatedCart);

    return {
      ...state,
      cart: updatedCart,
      cartQuantity: state.cartQuantity + 1,
      cartTotal: state.cartTotal + product.price,
    };
  }

  // reduce quantity
  if (action.type == actions.REDUCE_QUANTITY) {
    const product = state.cart.find((item) => item.id == action.product);
    if (!product || product.quantity <= 1) {
      return state;
    }
    const updatedCart = state.cart.map((item) =>
      item.id === product.id
        ? { ...item, quantity: item.quantity - 1 }
        : item
    );

    localforage.setItem("cartItems", updatedCart);

    return {
      ...state,
      cart: updatedCart,
      cartQuantity: Math.max(0, state.cartQuantity - 1),
      cartTotal: state.cartTotal - product.price,
    };
  }

  if (action.type == actions.SET_QUANTITY) {
    const product = state.cart.find((item) => item.id == action.product);
    if (!product) {
      return state;
    }
    const nextQuantity = Math.max(1, Number(action.quantity) || 1);
    const quantityDiff = nextQuantity - product.quantity;
    const updatedCart = state.cart.map((item) =>
      item.id === product.id
        ? { ...item, quantity: nextQuantity }
        : item
    );

    localforage.setItem("cartItems", updatedCart);

    return {
      ...state,
      cart: updatedCart,
      cartQuantity: Math.max(0, state.cartQuantity + quantityDiff),
      cartTotal: state.cartTotal + product.price * quantityDiff,
    };
  }

  // clear cart
  if (action.type == actions.CLEAR_CART) {
    localforage.setItem("cartItems", []);
    
    return {
      ...state,
      cart: [],
      order: [],
      cartTotal: 0,
      cartQuantity: 0,
    };
  }

  return state;
};

const useStore = () => {
  const [state, dispatch] = useReducer(reducer, initialState);

  const addToCart = (product, quantity = 1) => {
    // TODO: Add logic here and remove modification from dispatch
    dispatch({ type: actions.ADD_TO_CART, product, quantity });
  };

  const removeFromCart = (product) => {
    // TODO: Add logic here and remove modification from dispatch
    dispatch({ type: actions.REMOVE_FROM_CART, product });
  };

  const clearCart = () => {
    dispatch({ type: actions.CLEAR_CART });
  };
  const addProduct = async (product) => {
    try {
        await fetch(`${apiBaseUrl}/products`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(product),
      });


      // Nếu thành công, dispatch để cập nhật local state
      dispatch({ type: actions.ADD_PRODUCT, product: product});

      toast.success("Product added successfully!");
    } catch (error) {
      console.error("Add Product Error:", error);
      toast.error("Failed to add product");
    }
  };
  const getProducts = () => {
    fetch(`${apiBaseUrl}/products`)
      .then(async (response) => {
        const data = await response.json();
        let modifiedData = data.map((product) => {
          return { ...product, addedToCart: false };
        });
        let cart = (await localforage.getItem("cartItems")) || [];
        dispatch({
          type: actions.GET_PRODUCTS,
          products: modifiedData,
          backed_up_cart: cart,
        });
      })
      .catch((err) => {
        toast.error(
          "There was a problem fetching products, check your internet connection and try again"
        );
        return [];
      });
  };
  // DÒNG BẮT ĐẦU THAY ĐỔI
  const getProductsByPage = async (page = 0, size = 9) => {
    try {
      const response = await fetch(
        `${apiBaseUrl}/products?page=${page}&size=${size}`
      );
      
      if (!response.ok) throw new Error("API Response Error");
      
      const data = await response.json();
      const cart = (await localforage.getItem("cartItems")) || [];
      const rawProducts = data.content ? data.content : (Array.isArray(data) ? data : []);
      if (rawProducts.length === 0) {
        console.warn("Cảnh báo: Backend trả về danh sách sản phẩm trống !")
      }
      // 3. Map dữ liệu và so khớp với giỏ hàng
      const productsWithFlag = rawProducts.map((product) => {
        const isInCart = cart.some((item) => item.id === product.id);
        return {
          ...product,
          addedToCart: isInCart,
        };
      });
  
      // 4. Dispatch với đầy đủ thông tin giỏ hàng backup
      dispatch({
        type: actions.GET_PRODUCTS,
        products: productsWithFlag,
        backed_up_cart: cart, 
      });
  
      return data; 
    } catch (error) {
      console.error("Pagination Error:", error);
      toast.error("Error fetching paginated products");
      return null;
    }
  };
  
  
  
  const addQuantity = (product) => {
    dispatch({ type: actions.ADD_QUANTITY, product });
  };

  const reduceQuantity = (product) => {
    dispatch({ type: actions.REDUCE_QUANTITY, product });
  };

  const setQuantity = (product, quantity) => {
    dispatch({ type: actions.SET_QUANTITY, product, quantity });
  };

  const confirmOrder = async (order) => {
    let payload = {
      items: state.cart,
      totalItemCount: state.cartQuantity,
      delivery_type: order.DeliveryType,
      delivery_type_cost: order.DeliveryTypeCost,
      cost_before_delivery_rate: state.cartTotal,
      cost_after_delivery_rate: order.costAfterDelieveryRate,
      promo_code: order.promo_code || "",
      contact_number: order.phoneNumber,
      user_id: order.user_id,
      paymentMethod: order.paymentMethod,
    };

    const response = await fetch(`${apiBaseUrl}/order/place-order`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      mode: "cors",
      credentials: "include",
      body: JSON.stringify(payload),
    });

    const data = await response.json();
    if (!response.ok) {
      if (response.status === 401) {
        toast.error("You must be logged in to place an order");
        return { showRegisterLogin: true };
      }
      toast.error(data?.message || data?.error || "Failed to place order");
      return { showRegisterLogin: false };
    }

    if (order.paymentMethod === "VNPAY") {
      const amount = order.costAfterDelieveryRate * 1000000;
      const paymentRes = await fetch(
        `${apiBaseUrl}/payment/create-payment?amount=${amount}`,
        {
          method: "GET",
          credentials: "include",
        }
      );

      const paymentData = await paymentRes.json();

      if (paymentData.url) {
        window.location.href = paymentData.url;
      }else {
        toast.error("Không thể tạo link thanh toán VNPay");
      }
      return ;
    }

    toast.success("Order successful. Pls check your email");
    clearCart();
    return true;
  };

  return {
    state,
    addToCart,
    removeFromCart,
    clearCart,
    // getProducts,
    addProduct,
    addQuantity,
    reduceQuantity,
    setQuantity,
    getProductsByPage,
    confirmOrder,
  };
};

export default useStore;
